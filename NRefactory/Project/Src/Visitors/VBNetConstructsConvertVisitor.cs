	...
	public class VBNetConstructsConvertVisitor : ConvertVisitorBase
	{
		...
		public override object VisitMethodDeclaration(MethodDeclaration md, object data)
		{
			if (!IsClassType(ClassType.Interface) && (md.Modifier & Modifiers.Visibility) == 0)
				md.Modifier |= Modifiers.Public;
			
			if ("Finalize".Equals(md.Name, StringComparison.InvariantCultureIgnoreCase)
			    && md.Parameters.Count == 0
			    && md.Modifier == (Modifiers.Protected | Modifiers.Override)
			    && md.Body.Children.Count == 1)
			{
				TryCatchStatement tcs = md.Body.Children[0] as TryCatchStatement;
				if (tcs != null
				    ...
				    && tcs.FinallyBlock.Children.Count == 1)
				{
					...
					if (se != null) {
						...
						{
							DestructorDeclaration des = new DestructorDeclaration("Destructor", Modifiers.None, md.Attributes);
			...
			if ((md.Modifier & (Modifiers.Static | Modifiers.Extern)) == Modifiers.Static
			    && md.Body.Children.Count == 0)
			{
			...
			if (md.TypeReference.Type != "System.Void" && md.Body.Children.Count > 0) {
				if (IsAssignmentTo(md.Body.Children[md.Body.Children.Count - 1], md.Name))
				{
					ReturnStatement rs = new ReturnStatement(GetAssignmentFromStatement(md.Body.Children[md.Body.Children.Count - 1]).Right);
					md.Body.Children.RemoveAt(md.Body.Children.Count - 1);
					md.Body.AddChild(rs);
				} else {
					ReturnStatementForFunctionAssignment visitor = new ReturnStatementForFunctionAssignment(md.Name);
					md.Body.AcceptVisitor(visitor, null);
					if (visitor.replacementCount > 0) {
						Expression init;
						init = ExpressionBuilder.CreateDefaultValueForType(md.TypeReference);
						md.Body.Children.Insert(0, new LocalVariableDeclaration(new VariableDeclaration(FunctionReturnValueName, init, md.TypeReference)));
						md.Body.Children[0].Parent = md.Body;
						md.Body.AddChild(new ReturnStatement(new IdentifierExpression(FunctionReturnValueName)));
					}
				}
			}
		...
		{
			base.VisitBinaryOperatorExpression(bo, data);
			if (IsEmptyStringLiteral(bo.Right)) {
				if (bo.Op == BinaryOperatorType.Equality) {
					ReplaceCurrentNode(CallStringIsNullOrEmpty(bo.Left));
				} else if (bo.Op == BinaryOperatorType.InEquality) {
					ReplaceCurrentNode(new UnaryOperatorExpression(CallStringIsNullOrEmpty(bo.Left),
					                                               UnaryOperatorType.Not));
				}
			} else if (IsEmptyStringLiteral(bo.Left)) {
				if (bo.Op == BinaryOperatorType.Equality) {
					ReplaceCurrentNode(CallStringIsNullOrEmpty(bo.Right));
				} else if (bo.Op == BinaryOperatorType.InEquality) {
					ReplaceCurrentNode(new UnaryOperatorExpression(CallStringIsNullOrEmpty(bo.Right),
					                                               UnaryOperatorType.Not));
				}
			}
			return null;
		...
		public override object VisitLocalVariableDeclaration(LocalVariableDeclaration lvd, object data)
		{
			if (AddDefaultValueInitializerToLocalVariableDeclarations) {
				for (int i = 0; i < lvd.Variables.Count; i++) {
					VariableDeclaration decl = lvd.Variables[i];
					if (decl.FixedArrayInitialization.IsNull && decl.Initializer.IsNull) {
						TypeReference type = lvd.GetTypeForVariable(i);
