...
namespace ICSharpCode.NRefactory.Visitors
{
	...
	public class CSharpConstructsConvertVisitor : ConvertVisitorBase
	{
		...
		{
			if (bo.Op == BinaryOperatorType.Equality || bo.Op == BinaryOperatorType.InEquality) {
				if (IsNullLiteralExpression(bo.Left)) {
					Expression tmp = bo.Left;
					bo.Left = bo.Right;
					bo.Right = tmp;
				}
				if (IsNullLiteralExpression(bo.Right)) {
					if (bo.Op == BinaryOperatorType.Equality) {
						bo.Op = BinaryOperatorType.ReferenceEquality;
					} else {
						bo.Op = BinaryOperatorType.ReferenceInequality;
					}
				}
			}
			return base.VisitBinaryOperatorExpression(bo, data);
		}
