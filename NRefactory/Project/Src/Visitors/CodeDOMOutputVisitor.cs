...
namespace ICSharpCode.NRefactory.Visitors
{
	public class CodeDomVisitor : AbstractAstVisitor
	{
		...
		public override object VisitVariableDeclaration(VariableDeclaration vd, object data)
		{
			return null;
		}
		...
		public override object VisitMethodDeclaration(MethodDeclaration md, object data)
		{
			...
			memberMethod.Name = md.Name;
			memberMethod.Attributes = ConvMemberAttributes(md.Modifier);

			// RG: Private Interface Decl
			if ((memberMethod.Attributes & MemberAttributes.Public) != MemberAttributes.Public &&
			    md.InterfaceImplementations.Count > 0)
			{
				memberMethod.PrivateImplementationType = ConvType(md.InterfaceImplementations[0].InterfaceType);
			}
			...
			foreach (ParameterDeclarationExpression parameter in md.Parameters)
			{
			...
			Breakable.NextId = 0;
			variables.Clear();
			md.Body.AcceptChildren(this, data);
		...
		public override object VisitLocalVariableDeclaration(LocalVariableDeclaration lvd, object data)
		{
			...
			for (int i = 0; i < lvd.Variables.Count; ++i) {
				CodeTypeReference type = ConvType(lvd.GetTypeForVariable(i) ?? new TypeReference("System.Object", true));
				VariableDeclaration var = (VariableDeclaration)lvd.Variables[i];
		...
		public override object VisitReturnStatement(ReturnStatement rs, object data)
		{
			CodeMethodReturnStatement returnStmt;
			if (rs.Expression.IsNull)
				returnStmt = new CodeMethodReturnStatement();
			else
				returnStmt = new CodeMethodReturnStatement((CodeExpression)rs.Expression.AcceptVisitor(this,data));
		...
		{
			return new CodePrimitiveExpression(pe.Value);
		}
		...
		public override object VisitBinaryOperatorExpression(BinaryOperatorExpression bo, object data)
		{
			...
			System.Diagnostics.Debug.Assert(!bo.Left.IsNull);
			System.Diagnostics.Debug.Assert(!bo.Right.IsNull);

			var cboe = new CodeBinaryOperatorExpression(
				(CodeExpression)bo.Left.AcceptVisitor(this, data),
				op,
				(CodeExpression)bo.Right.AcceptVisitor(this, data));
			if (bo.Op == BinaryOperatorType.InEquality) {
