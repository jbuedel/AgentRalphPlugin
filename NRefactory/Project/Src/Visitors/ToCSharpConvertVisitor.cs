...
namespace ICSharpCode.NRefactory.Visitors
{
	...
	public class ToCSharpConvertVisitor : ConvertVisitorBase
	{
		...
		{
			ConvertInterfaceImplementation(md);
		...
		{
			base.VisitLocalVariableDeclaration(lvd, data);
			if ((lvd.Modifier & Modifiers.Static) == Modifiers.Static) {
				INode parent = lvd.Parent;
				...
				if (parent != null) {
					...
					if (type != null) {
						...
						if (pos >= 0) {
							...
							field.TypeReference = lvd.TypeReference;
							field.Modifier = Modifiers.Static;
							field.Fields = lvd.Variables;
