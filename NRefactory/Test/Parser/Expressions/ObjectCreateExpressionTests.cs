...
namespace ICSharpCode.NRefactory.Tests.Ast
{
	...
	public class ObjectCreateExpressionTests
	{
		...
		{
			Assert.IsInstanceOf<NamedArgumentExpression>(e);
		...
		void CheckPointObjectCreation(ObjectCreateExpression oce)
		{
			...
			Assert.IsInstanceOf<PrimitiveExpression>(CheckPropertyInitializationExpression(oce.ObjectInitializer.CreateExpressions[0], "X"));
			Assert.IsInstanceOf<PrimitiveExpression>(CheckPropertyInitializationExpression(oce.ObjectInitializer.CreateExpressions[1], "Y"));
		}
		...
		public void CSharpNestedObjectInitializerForPreinitializedProperty()
		{
            ...
            Assert.IsInstanceOf<PrimitiveExpression>(CheckPropertyInitializationExpression(aie.CreateExpressions[0], "X"));
            Assert.IsInstanceOf<PrimitiveExpression>(CheckPropertyInitializationExpression(aie.CreateExpressions[1], "Y"));
		...
		public void CSharpCollectionInitializer()
		{
            ...
            Assert.IsInstanceOf<PrimitiveExpression>(oce.ObjectInitializer.CreateExpressions[0]);
            Assert.IsInstanceOf<PrimitiveExpression>(oce.ObjectInitializer.CreateExpressions[1]);
            Assert.IsInstanceOf<PrimitiveExpression>(oce.ObjectInitializer.CreateExpressions[2]);
		}
	...
	new Contact(additionalParameter) {
		...
		PhoneNumbers = { ""650-555-0199"", ""425-882-8080"" }
			...
			Assert.IsInstanceOf<IdentifierExpression>(oce.Parameters[0]);
            Assert.IsInstanceOf<PrimitiveExpression>(CheckPropertyInitializationExpression(oce.ObjectInitializer.CreateExpressions[0], "Name"));
		...
		public void CSharpAnonymousType()
		{
			...
			Assert.AreEqual(3, oce.ObjectInitializer.CreateExpressions.Count);
			Assert.IsInstanceOf<PrimitiveExpression>(CheckPropertyInitializationExpression(oce.ObjectInitializer.CreateExpressions[0], "Name"));
			Assert.IsInstanceOf<IdentifierExpression>(oce.ObjectInitializer.CreateExpressions[1]);
			Assert.IsInstanceOf<MemberReferenceExpression>(oce.ObjectInitializer.CreateExpressions[2]);
		}
		#endregion
		...
		public void VBNetAnonymousType()
		{
			...
			Assert.IsInstanceOf<PrimitiveExpression>(CheckPropertyInitializationExpression(oce.ObjectInitializer.CreateExpressions[0], "Id"));
			Assert.IsInstanceOf<NamedArgumentExpression>(oce.ObjectInitializer.CreateExpressions[1]);
		}
		...
		public void VBNetMemberInitializationTest()
		{
			...
			Assert.IsInstanceOf<PrimitiveExpression>(((NamedArgumentExpression)oce.ObjectInitializer.CreateExpressions[0]).Expression);
			Assert.IsInstanceOf<PrimitiveExpression>(((NamedArgumentExpression)oce.ObjectInitializer.CreateExpressions[1]).Expression);
		}
