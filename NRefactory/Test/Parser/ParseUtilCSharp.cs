...
namespace ICSharpCode.NRefactory.Tests.Ast
{
	public class ParseUtilCSharp
	{
		...
		public static T ParseGlobal<T>(string program, bool expectError, bool skipMethodBodies) where T : INode
		{
            ...
            if (expectError)
                Assert.IsTrue(parser.Errors.Count > 0, "There were errors expected, but parser finished without errors.");
            else
                Assert.IsTrue(parser.Errors.Count == 0, "Unable to parse the statement.  Parse errors:" + parser.Errors.ErrorOutput);
		...
		public static T ParseExpression<T>(string expr, bool expectErrors) where T : INode
		{
			...
			if (expectErrors)
				...
				Assert.IsTrue(parser.Errors.Count == 0, "Unable to parse the expression.  Parse errors:" + parser.Errors.ErrorOutput);
			Type type = typeof(T);
